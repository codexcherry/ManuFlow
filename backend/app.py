...
class BOM(db.Model):
    ...
    production_time = db.Column(db.Float, default=0.0)  # in minutes
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
@app.route('/api/work-centers/<int:center_id>', methods=['PUT'])
@login_required
def update_work_center(center_id):
    center = WorkCenter.query.get_or_404(center_id)
    data = request.get_json()
    
    center.name = data.get('name', center.name)
    center.description = data.get('description', center.description)
    center.cost_per_hour = data.get('cost_per_hour', center.cost_per_hour)
    center.capacity = data.get('capacity', center.capacity)
    center.is_active = data.get('is_active', center.is_active)
    
    db.session.commit()
    return jsonify({'message': 'Work center updated successfully'})

@app.route('/api/work-centers/<int:center_id>', methods=['DELETE'])
@login_required
def delete_work_center(center_id):
    center = WorkCenter.query.get_or_404(center_id)
    
    # Check if work center is used in any work orders
    existing_work_orders = WorkOrder.query.filter_by(work_center_id=center_id).first()
    if existing_work_orders:
        return jsonify({'error': 'Cannot delete work center that is used in work orders'}), 400
    
    db.session.delete(center)
    db.session.commit()
    
    return jsonify({'message': 'Work center deleted successfully'})
...
def get_boms():
    ...
    for bom in boms:
        ...
        result.append({
            ...
            'production_time': bom.production_time,
...
def create_bom():
    ...
    bom = BOM(
        ...
        production_time=data.get('production_time', 0.0)
    )
...
def update_bom(bom_id):
    ...
    # Update BOM details
    bom.name = data.get('name', bom.name)
    bom.description = data.get('description', bom.description)
    bom.quantity = data.get('quantity', bom.quantity)
    bom.production_time = data.get('production_time', bom.production_time)
    
    # Delete existing components
    BOMLine.query.filter_by(bom_id=bom_id).delete()
    ...
    db.session.commit()
    return jsonify({'message': 'BOM updated successfully'})
...
def delete_bom(bom_id):
    ...
    existing_orders = ManufacturingOrder.query.filter_by(bom_id=bom_id).first()
    if existing_orders:
        return jsonify({'error': 'Cannot delete BOM that is used in manufacturing orders'}), 400
...
def get_manufacturing_orders():
    ...
    for order in orders:
        result.append({
            ...
            'bom_id': order.bom_id,
            'bom_name': order.bom.name if order.bom else 'Unknown BOM',
...
def create_manufacturing_order():
    ...
    # Create work orders based on BOM production time
    bom = BOM.query.get(data.get('bom_id'))
    if bom and bom.production_time > 0:
        work_order = WorkOrder(
            manufacturing_order_id=order.id,
            work_center_id=1,  # Default work center, should be configurable
            operation_name=f"Produce {bom.product.name}",
            estimated_time=bom.production_time * data.get('quantity_to_produce'),
            assignee_id=data.get('assignee_id')
        )
...
def confirm_manufacturing_order(order_id):
    ...
    for component in bom.components:
        ...
        unit_cost = component.product.cost_price or 0.0
        total_value = required_qty * unit_cost
...
def complete_manufacturing_order(order_id):
    ...
    # Complete all associated work orders
    ...
    for work_order in work_orders:
        if work_order.state != 'completed':
            ...
            # If work order was started, calculate actual time
            if work_order.started_at:
                ...
                # If work order was never started, use estimated time as actual time
                work_order.actual_time = work_order.estimated_time
...
@app.route('/api/manufacturing-orders/<int:order_id>', methods=['DELETE'])
@login_required
def delete_manufacturing_order(order_id):
    order = ManufacturingOrder.query.get_or_404(order_id)
    
    # Delete associated work orders
    WorkOrder.query.filter_by(manufacturing_order_id=order_id).delete()
    
    # Delete the manufacturing order
    db.session.delete(order)
    db.session.commit()
    
    return jsonify({'message': 'Manufacturing order deleted successfully'})
...
def start_work_order(wo_id):
    ...
    # Update manufacturing order state if it's still planned
    manufacturing_order = ManufacturingOrder.query.get(work_order.manufacturing_order_id)
    if manufacturing_order and manufacturing_order.state == 'planned':
        manufacturing_order.state = 'in_progress'
        manufacturing_order.started_at = datetime.utcnow()
...
def complete_work_order(wo_id):
    ...
    if manufacturing_order:
        # Calculate progress based on work order completion
        # For now, we'll assume completing a work order means completing the full quantity
        manufacturing_order.quantity_produced = manufacturing_order.quantity_to_produce
        
        # Check if all work orders for this manufacturing order are completed
        remaining_work_orders = WorkOrder.query.filter(
            WorkOrder.manufacturing_order_id == manufacturing_order.id,
            WorkOrder.state != 'completed'
        ).count()
        
        if remaining_work_orders == 0:
            manufacturing_order.state = 'done'
...
# Sync Work Orders with Manufacturing Orders
@app.route('/api/work-orders/sync', methods=['POST'])
@login_required
def sync_work_orders():
    """
    Sync work orders with their manufacturing orders.
    This will complete work orders whose manufacturing orders are already completed.
    """
    try:
        # Find all completed manufacturing orders
        completed_manufacturing_orders = ManufacturingOrder.query.filter_by(state='done').all()
        
        synced_count = 0
        for mo in completed_manufacturing_orders:
            # Find all work orders for this manufacturing order that are not completed
            ...
            for work_order in work_orders:
                work_order.state = 'completed'
                work_order.completed_at = mo.completed_at or datetime.utcnow()
        ...
        return jsonify({
            'message': f'Successfully synced {synced_count} work orders with completed manufacturing orders',
            'synced_count': synced_count
        }), 200
        
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': f'Failed to sync work orders: {str(e)}'}), 500

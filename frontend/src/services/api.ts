...
export const workCentersAPI = {
  ...
  update: async (id: number, data: CreateWorkCenterData & { is_active?: boolean }): Promise<{ message: string }> => {
...
export const bomAPI = {
  ...
  update: async (id: number, data: CreateBOMData): Promise<{ message: string }> => {
...
export const workOrdersAPI = {
  ...
  sync: async (): Promise<{ message: string; synced_count: number }> => {
    const response = await api.post('/work-orders/sync');
    return response.data;
  },
};
